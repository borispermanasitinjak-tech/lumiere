<?php require_once APPROOT . '/app/views/layouts/header.php'; ?>

<div class="container" style="padding: 40px 20px;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
        <h2>Payment Methods</h2>
        <a href="<?php echo BASE_URL; ?>/admin/add_payment_method" class="btn btn-primary">
            <i class="fas fa-plus"></i> Add Payment Method
        </a>
    </div>

    <?php if(isset($_SESSION['flash_success'])): ?>
        <div style="background: #d4edda; border-left: 4px solid #28a745; padding: 15px; margin-bottom: 20px;">
            <?php echo e($_SESSION['flash_success']); unset($_SESSION['flash_success']); ?>
        </div>
    <?php endif; ?>

    <div class="card" style="padding: 0; overflow: hidden;">
        <table style="width: 100%; border-collapse: collapse;">
            <thead style="background: var(--background-color);">
                <tr>
                    <th style="padding: 15px; text-align: left;">Name</th>
                    <th style="padding: 15px; text-align: left;">Type</th>
                    <th style="padding: 15px; text-align: left;">Account Info</th>
                    <th style="padding: 15px; text-align: center;">Status</th>
                    <th style="padding: 15px; text-align: center;">Actions</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach($data['methods'] as $method): ?>
                    <tr style="border-bottom: 1px solid #eee;">
                        <td style="padding: 15px; font-weight: 600;"><?php echo e($method->name); ?></td>
                        <td style="padding: 15px;">
                            <span style="padding: 4px 10px; background: #e3f2fd; color: #1976d2; border-radius: 15px; font-size: 0.85rem;">
                                <?php echo ucfirst(str_replace('_', ' ', $method->type)); ?>
                            </span>
                        </td>
                        <td style="padding: 15px; font-size: 0.9rem;">
                            <?php if($method->account_name): ?>
                                <?php echo e($method->account_name); ?><br>
                                <?php echo e($method->account_number); ?>
                            <?php else: ?>
                                <span style="color: #999;">-</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding: 15px; text-align: center;">
                            <?php if($method->is_active): ?>
                                <span style="padding: 5px 15px; background: #d4edda; color: #155724; border-radius: 20px; font-size: 0.85rem;">Active</span>
                            <?php else: ?>
                                <span style="padding: 5px 15px; background: #f8d7da; color: #721c24; border-radius: 20px; font-size: 0.85rem;">Inactive</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding: 15px; text-align: center;">
                            <a href="<?php echo BASE_URL; ?>/admin/edit_payment_method/<?php echo $method->id; ?>" class="btn btn-outline" style="padding: 5px 10px; font-size: 0.85rem; margin-right: 5px;">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <a href="<?php echo BASE_URL; ?>/admin/delete_payment_method/<?php echo $method->id; ?>" 
                               onclick="return confirm('Delete this payment method?')" 
                               class="btn btn-outline" style="padding: 5px 10px; font-size: 0.85rem; color: #e74c3c; border-color: #e74c3c;">
                                <i class="fas fa-trash"></i>
                            </a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
</div>

<?php require_once APPROOT . '/app/views/layouts/footer.php'; ?>
